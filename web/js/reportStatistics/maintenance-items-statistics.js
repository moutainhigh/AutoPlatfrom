...
var maintain = '保养'
function isMaintainFix() {
    $("#isMaintainFix").bootstrapSwitch({
        onText: '保养',
        offText: '维修',
        onColor: 'success',
        offColor: 'danger',
...
function companyQuery(){
    getColumnarChart("columnar", "/maintainFix/query_default?maintain=" + maintain, tempData,"default","维修保养项目本月统计");
}
...
function search(count){
    if(count == 1){
        ...
        var end = $("#end1").val();
        validator(start,end,type,"维修保养项目年统计");
    }else if(count == 2){
        type = 'quarter'
        var start = $("#start2").val();
        var end = $("#end2").val();
        validator(start,end,type,'维修保养项目季度统计');
    }else if(count == 3){
        type = 'month'
        var start = $("#start3").val();
        var end = $("#end3").val();
        validator(start,end,type,'维修保养项目月统计');
    }else if(count == 4){
        type = 'week'
        var start = $("#start4").val();
        var end = $("#end4").val();
        validator(start,end,type,'维修保养项目周统计');
    }else if(count == 5){
        type = 'day'
        var start = $("#start5").val();
        var end = $("#end5").val();
        validator(start,end,type,'维修保养项目日统计');
    }
}
    ...
    function validator( start, end, type,text){
        if($("#span").text()== 'admin'){
            if($("#spans").text() != ''){
                if(start != '' && end != ''){
                    getLineBasicChart("columnar", "/maintainFix/query_condition?start=" + start +"&end=" + end + "&type=" + type + "&companyId="+companyId + "&maintain=" + maintain, tempData,type,text);
                }else{
                    getLineBasicChart("columnar", "/maintainFix/query_default?companyId="+companyId + "&maintain="+maintain, tempData,"default","维修保养项目本月统计");
                }
            }else{
                showCompany();
            }
            ...
            if(start != '' && end != ''){
                getColumnarChart("columnar", "/maintainFix/query_condition?start=" + start +"&end=" + end + "&type=" + type+ "&maintain=" + maintain, tempData,type,text);
            }else{
                getColumnarChart("columnar", "/maintainFix/query_default?maintain=" + maintain , tempData,"default","维修保养项目本月统计");
            }
        }
