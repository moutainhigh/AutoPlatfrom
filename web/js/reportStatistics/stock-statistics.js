...
function companyQuery(){
    getColumnarChart("columnar", "/accessories/query_default?quantity=" + quantity, tempData,"default","库存本月统计");
}
...
function search(count){
    if(count == 1){
        ...
        var end = $("#end1").val();
        validator(start,end,type,"库存年统计");
    }else if(count == 2){
        type = 'quarter'
        var start = $("#start2").val();
        var end = $("#end2").val();
        validator(start,end,type,'库存季度统计');
    }else if(count == 3){
        type = 'month'
        var start = $("#start3").val();
        var end = $("#end3").val();
        validator(start,end,type,'库存月统计');
    }else if(count == 4){
        type = 'week'
        var start = $("#start4").val();
        var end = $("#end4").val();
        validator(start,end,type,'库存周统计');
    }else if(count == 5){
        type = 'day'
        var start = $("#start5").val();
        var end = $("#end5").val();
        validator(start,end,type,'库存日统计');
    }
}
    ...
    function validator( start, end, type,text){
        if($("#span").text()== 'admin'){
            if($("#spans").text() != ''){
                if(start != '' && end != ''){
                    getLineBasicChart("columnar", "/accessories/query_condition?start=" + start +"&end=" + end + "&type=" + type + "&companyId="+companyId + "&quantity=" + quantity, tempData,type,text);
                }else{
                    getLineBasicChart("columnar", "/accessories/query_default?companyId="+companyId + "&quantity="+quantity, tempData,"default","库存本月统计");
                }
            }else{
                showCompany();
            }
            ...
            if(start != '' && end != ''){
                getColumnarChart("columnar", "/accessories/query_condition?start=" + start +"&end=" + end + "&type=" + type+ "&quantity=" + quantity, tempData,type,text);
            }else{
                getColumnarChart("columnar", "/accessories/query_default?quantity=" + quantity , tempData,"default","库存本月统计");
            }
        }
